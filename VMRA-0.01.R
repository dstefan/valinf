...
# TO DO: expose the individual functions for indepedent use
# TO DO: add value of info wrt risk
...
vmra = function(value, cost=0, must=0, labels=NULL){
    ...
    EVPI.cost = rep(NA, M)
    names(EVPI.cost) = labels 
    for (i in 1:M){
      EVPI.cost[i] = evpi(cost[,i], NB)
    }
    

    # Perfect Information about Values
    EVPI.value = rep(NA, M)
    names(EVPI.value) = labels 
    for (i in 1:M){
      EVPI.value[i] = evpi(value[,i], NB)
    }
    
    # TO DO: add computation of perfect information about cost and value relative to risk!
    # ....
